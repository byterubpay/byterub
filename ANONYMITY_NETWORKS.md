# Anonymity Networks with ByteRub

Currently only Tor and I2P have been integrated into ByteRub. The usage of
...
is made or until `byterubd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `byterub-wallet-cli` and
`byterub-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `byterubd` the ip/port of a socks proxy provided by a
...
which tells `byterubd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `byterubd` the inbound address, network
...
which tells `byterubd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `byterubd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `byterubd` localhost port 30000.
...
`byterubd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/byterub
...
This will store key information in `/var/lib/tor/data/byterub` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/byterub/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the ByteRub network should have
similar timestamps.
...
### Intermittent ByteRub Syncing

If a user only runs `byterubd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `byterubd` as often as possible to conceal when transactions are being sent.
...
`byterubd` currently selects two outgoing connections every 5 minutes for
...
`byterubd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `byterubd` should likely disconnect
