...
Core tests take longer than any other ByteRub tests, due to the high amount of computational work involved in validating core components.
...
To run only ByteRub's core tests (after building):
...
To run only ByteRub's crypto tests (after building):
...
```bash
byterubd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `byterub-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only ByteRub's hash tests (after building):
...
To run only ByteRub's performance tests (after building):
...
To run only ByteRub's unit tests (after building):
