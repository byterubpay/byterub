...
*Setup instructions for a Gitian build of ByteRub.*

Gitian is the deterministic build process that is used to build the ByteRub CLI
...
```bash
cp byterub/contrib/gitian/gitian-build.py .
```
...
fork the [gitian.sigs repository](https://github.com/byterubpay/gitian.sigs) and clone it on your host machine, 
or pass the signed assert file back to your build machine.

```bash
git clone git@github.com:byterubpay/gitian.sigs.git
...
You can also look in the [gitian.sigs](https://github.com/byterubpay/gitian.sigs/) repo and / or [getbyterub.org release checksums](https://web.getbyterub.org/downloads/hashes.txt) to see if others got the same checksum for the same version tag.  If there is ever a mismatch -- **STOP! Something is wrong**.  Contact others on IRC / github to figure out what is going on.
...
gpg --detach-sign ${VERSION}-linux/${GH_USER}/byterub-linux-*-build.assert
gpg --detach-sign ${VERSION}-win/${GH_USER}/byterub-win-*-build.assert
gpg --detach-sign ${VERSION}-osx/${GH_USER}/byterub-osx-*-build.assert
gpg --detach-sign ${VERSION}-android/${GH_USER}/byterub-android-*-build.assert
```
...
[byterubpay/gitian.sigs](https://github.com/byterubpay/gitian.sigs/) repository:
...
**Note:** Please ensure your gpg public key is available to check signatures by adding it to the [gitian.sigs/gitian-pubkeys/](https://github.com/byterubpay/gitian.sigs/tree/master/gitian-pubkeys) directory in a pull request.
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/byterubmooo-byterub/bitbyterub -b byterubmooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
